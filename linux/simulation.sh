...
ea_down() {
  if ea_is_mgmt; then
    echo "Blocked ethernet shutdown — management IP active on $eadapter" | tee -a /usr/local/scripts/sim.log
...
report_error() {
  ...
  echo "$(date '+%Y-%m-%dT%H:%M:%S') [$severity] $msg" | tee -a /usr/local/scripts/sim.log
